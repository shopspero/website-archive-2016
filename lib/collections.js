	...
	*/
	...
	Meteor.publish('items', function(category, subcategory) {
		...
		if (category && subcategory) {
			return Items.find({category: category.name, "subcategories.query" : subcategory},  { sort: { createdAt: -1 }} )
		} else if (category) {
			return Items.find({category: category.name},  { sort: { createdAt: -1 }} )
		} else {
			return Items.find({" subcategories.query" : subcategory},  { sort: { createdAt: -1 }} )
		}		
	})
